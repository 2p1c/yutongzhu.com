...
import type { PostListItem } from '../lib/post-storage.js'

function renderPostList(posts: PostListItem[], showSource = false) {
  return html`<ul class="post-list">
    ${
        ...
        : posts.map(post => {
            ...
            const source =
              showSource && post.sourceUrl && post.sourceTitle
                ? html`<p class="post-list-source"><em>source:</em> <a href="${post.sourceUrl}" target="_blank" rel="noopener noreferrer">${post.sourceTitle}</a></p>`
                : ''
            return html`
                <li class="post-item">
...
export function renderHomeBody(posts: PostListItem[]) {
  ...
  <section class="bio">
    ...
    <p><span class="lang-en">The only goal is to find inner peace, build my own identity outside the rules dictated by the world. Learning, building, dreaming, never give up.</span><span class="lang-zh">唯一的目标是找到内心的平静，在被社会告知的规则之外建立自己的身份。学习，构建，幻想，永不放弃。</span></p>  
    <p><span class="lang-en">My primary technical expertise is lies in eat, sleep, workout, dream and computer games</span><span class="lang-zh">我的主要技术专长在于吃、睡、健身、做梦，以及电脑游戏。</span></p>
  </section>
